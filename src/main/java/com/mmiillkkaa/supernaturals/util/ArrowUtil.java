...
public class ArrowUtil implements Runnable {
    ...
    public void run() {
        if (arrow != null) {
          ...
          * else { if (player.getLocation().distance(targetLocation) < 3) {
          * SupernaturalsPlugin
          * .instance.getHunterManager().stopGrappling(player); return; }
          * 
          * if (startTime + 500 + grappleDistance * 70 <
          * System.currentTimeMillis()) {
          * SupernaturalsPlugin.instance.getHunterManager
          * ().stopGrappling(player); return; }
          * 
          * Vector travelVector =
          * targetLocation.toVector().subtract(player.getLocation
          * ().toVector()).normalize();
          * 
          * // Always move slightly upwards to combat the evil force of gravity!
          * travelVector = travelVector.setY(travelVector.getY() +
          * 0.2).normalize();
          * 
          * // set travel speed travelVector = travelVector.multiply(1.0);
          * player.setVelocity(travelVector); ArrowUtil gh = new
          * ArrowUtil(player, targetLocation, startTime, grappleDistance);
          * SupernaturalsPlugin
